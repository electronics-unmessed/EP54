...
int speed = 22;    // ??_1: Rounds per minute (max 30)
...
float Gain = 1.0;            // Gain of Gauge to calibrate
float Brightness = 155;      // Brightness for LEDs PWM 0..255
float Max_Brightness = 255;  // Brightness for LEDs for Clipping, Start etc PWM 0..255
...
void Start_Up() {
  ...
  if (count < 5000) { val = 180 * val_A2 / 1023 + diff; };  // shows rel Resonance
  if (count < 4000) { val = 180 * val_A1 / 1023 + diff; };  // shows rel Damping
  if (count < 3000) { val = 180 * val_A4 / 1023 + diff; };  // shows rel Gain

  if (count < 2000) { val = 180 + diff; };
...
void Lights() {
  if (pos < 181 + zero_offset + zero_adjust) {
    analogWrite(LED_Lights, Brightness);
  } else {
    analogWrite(LED_Lights, Max_Brightness);
  };
}
