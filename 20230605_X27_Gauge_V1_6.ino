...
//       Potentiometers for gain, zero adjust, filter damping, fiter resonance frequency, LED brightness
...
long run_time = 200000;   // ??_5: Runtime in case audio < threshold
float threshold = 4.0;     // ??_4: threshold for "wake-up"
long run_count = 200000;  // counter for runtime count down

int pos = 0;  //Position in steps(0-630)= (0°-315°)
int val = 0;  // to be Position in Steps 0..180 = 90°

int speed = 10;    // ??_1: Rounds per minute (max 30)
int Delay = 8;  // ??_2: 10-30 RPM 8333us OK with microphone
float diff = 1.0;  // ??_3: 0..10 difference val-pos for stepper to get active

float no_average = 1000;  // averaging for audio reference level
...
float Gain = 1;          // Gain of Gauge to calibrate
float Brightness = 255;  // Brightness for LEDs PWM 0..255
...
void loop() {
  ...
  if (run_count > 2) {
    ...
    // FilterInput_0 = Gain * audio_max_log;  // for normal operation
    FilterInput_0 = 2 * Gain * audio_max_log;  // Testing
...
void Start_Up() {
  ...
  if (count < 5 * no_average) { val = 180 * val_A2 / 1023 + diff; };  // shows rel Resonance
  if (count < 4 * no_average) { val = 180 * val_A1 / 1023 + diff; };  // shows rel Damping
  if (count < 3 * no_average) { val = 180 * val_A4 / 1023 + diff; };  // shows rel Gain

  if (count < 2 * no_average) { val = 180 + diff; };
  if (count < 1 * no_average) { val = 0 + diff; };
  if (count > 20 * no_average) { count = 22 * no_average; };
}
